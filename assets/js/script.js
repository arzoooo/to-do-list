...
$(document).ready(function(){
    ...
    // getting all the class name category 
        for(let i=0;i<categorys.length;i++){ 
            // looping in the  categorys to find the which category class belongs and implement according design check home.css to get the color of each section
...
function checkedOrNot(){ 
    ...
    // getting all the check-box class 
    let descdisp = document.querySelectorAll('.dispdsc'); 
    // gettong all the class where descripting of TODO is defined
    let ddsp = document.querySelectorAll('.dueDate'); 
    // getting all the class for dueDate
    ...
    for(let i=0;i<descdisp.length;i++){
        ...
        // checking if checkbox is checked  if checked a line will pass through the text(-) else if it is unchecked no line will pass through date and description
            ...
            document.getElementById(cb[i].getAttribute('uid')+d).style.textDecoration  = 'line-through'
            }
            else if(cb[i].checked == false){
            document.getElementById(cb[i].getAttribute('uid')).style.textDecoration = 'none'
            document.getElementById(cb[i].getAttribute('uid')+d).style.textDecoration  = 'none'
        }
...
document.getElementById('deleteButton').addEventListener('click',function(){
    ...
    // getting only checked vale
    let arrcheck = []  
    // creating the list of checked array
    for(let i of checedvaluew){
        let gg=''
       gg= i.getAttribute('uid')   
        // getting uniue id from and pushing into array
        console.log(gg)
        arrcheck.push(gg);
    }
    ...
    $.ajax({
        ...
        success: function(){ 
            // on ajax succes i.e when data is delete
            swal("Item is deleted ", "click ok to go back Home ", "success") // using  alert to show the data is delete
            .then(redir => {
                window.location = '/';
            })
